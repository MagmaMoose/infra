...
module "platform" {
  ...
  db_mode      = "external"
  database_url = "postgresql://dunmir:dunmir@postgres:5432/dunmir" # checkov:skip=CKV_SECRET_6:Local Postgres test credential — LocalStack only, not a real password # trivy:ignore:postgresql-url # nosemgrep
