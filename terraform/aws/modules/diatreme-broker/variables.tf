...
variable "environment" {
  description = <<-EOT
    Environment segment of the SSM secret path, e.g. `/diatreme/prod/app-id`. Supplied by the
...
variable "oidc_audience" {
  description = <<-EOT
    ...
    the other service's identity: brimyr's client asks for `aud=brimyr` and diatreme's for
    `aud=diatreme`, and a token carrying the wrong one is rejected as `invalid_oidc` before
    any App JWT is signed. Two services must never share a value.

    Empty (the default) means "use name_prefix", which is exactly what diatreme was doing when
    this was hardcoded — so diatreme's rendered configuration is unchanged by parameterising it.
  EOT
  type        = string
  default     = ""
}
...
variable "artifact_prefix" {
  description = <<-EOT
    Key prefix inside the artifact bucket. Must match both what diatreme's release workflow
...
variable "broker_artifact_version" {
  description = <<-EOT
    ...
    THIS LINE IS THE DEPLOYMENT. Diatreme's release workflow publishes the object and prints
    ...
    Diatreme only publishes when the zip's inputs actually CHANGED (the release workflow gates
    on `build_lambda_zip.py --changed-since <previous tag>`). Most diatreme releases are CLI
...
variable "domain_name" {
  description = <<-EOT
    Custom hostname for the broker, e.g. "broker-diatreme.magmamoose.com".

    MUST BE A FIRST-LEVEL SUBDOMAIN if it is to be Cloudflare-proxied on the free plan:
    Universal SSL covers the apex and one label only, so `broker.diatreme.magmamoose.com`
    would need Advanced Certificate Manager (~$10/month) while `broker-diatreme…` is free.
