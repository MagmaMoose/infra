...
> than being durable~~ — **superseded 2026-08-07 (#588):** `topology.sargeant.co/{role,tier}` is now
> self-registered from each node's k3s config, so placement survives node re-registration. Only
> `node-role.kubernetes.io/*` still needs the playbook, because NodeRestriction makes a kubelet REFUSE
> registration rather than skip those.
...
> `status: error`. **RESOLVED 2026-08-08 (#604).** The providers, applications and the
> embedded-outpost binding are now in git as a blueprint ConfigMap named in
> `blueprints.configMaps`, and were created against the live instance to prove the
> shape first: `/outpost.goauthentik.io/auth/traefik` answers HTTP 302 for both
> `defectdojo.magmamoose.com` and `headlamp.sargeant.co`. A Google OAuth Source also
> exists (the redirect URI is registered), so identity is no longer the gap either.
>
...
- [x] ~~simplify labels for node-pinning.~~ **DONE.** **New labels APPLIED to the live nodes 2026-08-07** — additive, nothing selects them yet. The insight that makes the taxonomy work: **role and location are orthogonal axes**, which dissolves the old complaint that `worker` spans both tiers. Verified resolution against the live cluster:
  ...
  - [x] ~~**DURABILITY GAP~~ **CLOSED 2026-08-07 (#588).** `ansible/firefly-node-label-durability.yaml` writes `topology.sargeant.co/{role,tier}` into every node's k3s config drop-in, validated on each host and idempotent. `node-role.kubernetes.io/*` deliberately stays API-applied — NodeRestriction makes a kubelet REFUSE registration rather than skip them. Original text: **DURABILITY GAP — applied with imperative `kubectl label`, so they are LOST on node re-registration.** The kubelet may not self-register `node-role.kubernetes.io/*` (NodeRestriction), so they must be baked into each host's k3s config (`node-label:` in `/etc/rancher/k3s/config.yaml` for ff-pi1, `config.yaml.d/` for agents). The OCI pair sets labels via cloud-init in `terraform/oci/modules/server` — changing that REPLACES the VM. ff-vm1 is not in the Ansible inventory.
  - [x] ~~remove the OLD labels~~ **DONE 2026-08-08.** `type`, `node-role.kubernetes.io/pi` and `.../mini` removed from ff-pi1 and ff-vm1 after verifying zero live consumers: no controller (Deployment/StatefulSet/DaemonSet/CronJob) selects them, and the single pod that did was a `Completed` CronJob pod from before the migration. Original text: remove the OLD labels (`type=pi`, `type=mini`, `node-role.kubernetes.io/{pi,mini}`) only AFTER all ~40 consumers migrate off `components/node-selectors` — see the Kyverno item below.
- [x] ~~implement "priority classes" (see appendix):~~ **DONE.** All five exist with the appendix values (`critical` 100000 / `business` 50000 / `high` 10000 / `medium` 1000 / `low` 100), assigned via Kyverno mutation, and every workload carrying one resolves to the correct `.spec.priority` integer.
  - `critical`
  - `business`
  - `high`
  - `medium` (NOT `globalDefault` — see appendix; unlabelled pods stay at priority `0` on purpose)
  - `low`
  - [x] **`business` sits between `high` and `critical` deliberately.** It encodes a different axis from the rest of the scale — the others rank by *blast radius* ("what else breaks?"), this one ranks by *revenue impact* ("who is affected?"). Mixing axes in one ordinal scale is only safe under a single rule, below. Business apps rank **below** `critical` on purpose: the secrets, TLS, admission and data planes are what they run on, so a business app that outranks them wins a scheduling race and then fails anyway.
  - [x] **Rule: nothing may outrank its own dependencies.** Otherwise under contention an app preempts the very thing it needs and takes itself down — a self-inflicted outage that looks like a mystery. Verified against live pod env today:
  ...
  - [x] **Only 3 of the 9 candidate apps are cluster workloads.** A PriorityClass is a pod-scheduling construct and has no meaning for the rest:
    ...
    - no manifest, namespace or workflow reference anywhere yet: **brimyr**, **draventis**, **tremvok**, **noctyr**
  ...
  - [x] **keep the custom scale below `longhorn-critical`.** If the storage layer loses a scheduling race, every stateful workload above it fails anyway — storage should win. The appendix values (100000 → 100) sit deliberately beneath it, with roughly 10× gaps so a tier can be inserted later without renumbering.
  ...
  - [x] apply `priorityClassName` via a per-tier Kyverno mutation, rather than editing ~90 pod specs by hand — **but the policy must target the controller kinds (`Deployment`/`StatefulSet`/`DaemonSet`/`CronJob` pod templates), never bare `Pod`.** The kube-apiserver's in-tree `Priority` admission plugin runs *before* `MutatingAdmissionWebhook` and is what resolves `priorityClassName` into the integer `spec.priority` that the scheduler actually reads. A webhook that stamps the name onto an already-admitted Pod leaves `spec.priority` unchanged — the field would *look* right in `kubectl get pod -o yaml` while every scheduling and preemption decision ignored it. Mutating the controller's template avoids this, because the resulting Pod is admitted with the name already present. Prove it with a canary before rolling out: assert `.spec.priority == 100000`, not just the name.
  - [x] note: names with a `system-` prefix are **rejected by the API server** (`priority class names with 'system-' prefix are reserved for system use only`) — verified against the live cluster. This is why the list above uses bare names.
- [x] ~~**repoint the cloud-tier business apps off the on-prem cache.**~~ **DONE.** `nievah` and `caldrith` now use `valkey-oci`; `litellm` and `browserless` were also moved to the cloud tier (#595). `nievah` and `caldrith` run on the cloud tier but both set `REDIS_URL=redis://valkey.database.svc.cluster.local:6379` — the `valkey` instance pinned to **ff-vm1**, the most contended and least available node in the fleet (~99% CPU / ~100% memory requested), reached across the site-to-site VPN. A cloud-tier `valkey-oci` already exists for exactly this. As it stands, the one tier that can survive a node failure has a hard dependency on the one node most likely to have one, which undoes the whole point of putting these apps in `cloud`. Repoint both at `valkey-oci.database.svc`, then drop `valkey` back to `high`.
- [x] ~~add the other two legs of availability~~ **DONE (#577).** 7 PodDisruptionBudgets and topology spread now in-repo. — priority class alone does not make a workload available:
  - [x] ~~**PodDisruptionBudgets — corrected.**~~ An earlier draft of this item was wrong on every example and would have caused harm; keeping the correction visible. All 18 PDBs are **operator-generated — not one exists in this repo**, so none can be "fixed" by editing a manifest:
  ...
  - [x] ~~**Spread.**~~ Replica count without spread is not fault tolerance — two replicas on one node die together. Only `kube-system/coredns` has `topologySpreadConstraints` today. The in-repo precedent to copy is `postgres-oci`, which uses required pod anti-affinity on `kubernetes.io/hostname` to guarantee one instance per OCI node.
- [x] ~~Replace `components` with Kyverno mutating policies.~~ **DONE.** `components/node-selectors` and `components/resource-profiles` are both deleted; placement is one `placement.sargeant.co/tier` label mutated at admission. Deleting the tree outright would drop node pinning entirely — `components/node-selectors/*` is currently the only thing that puts a `nodeSelector` on a workload's pod spec, so "delete and inline by hand" trades one problem for ~90 hand-maintained selectors. Instead:
  - [x] one `ClusterPolicy` per tier (`system`, `worker`, `on-prem`, `cloud`) that mutates the pod spec based on a namespace or workload label, e.g. `topology.sargeant.co/tier: cloud`. Policies live alongside the existing ones in `kubernetes/apps/kyverno-policies/base/`.
  - [x] pinning then becomes one label on the app, not a `components:` reference plus a patch target per workload kind (Deployment/StatefulSet/DaemonSet/CronJob are four separate patches in every component today).
  - [x] migrate the ~90 workloads off `components/node-selectors`, then delete `components/node-selectors`, `components/resource-profiles` (see below), and audit the rest (`gluetun-sidecar`, `vpn-routed-proxy`, `wireguard-sidecar`, `pod-gateway-client`, `helm-release-standards`, `helm-releases`, `ingress-standards`) individually — some are genuinely sidecar-injection, which is also Kyverno's job.
  - [x] note this makes Kyverno load-bearing for scheduling: if the admission webhook is down, new pods land unpinned. Keep `failurePolicy` and the kyverno replica count deliberate.
- [x] ~~Under `infrastructure > controllers`, the "stack" folder needs to be removed.~~ **DONE.** Both `stack/` levels are gone. The items within the folder need to be put directly within the controllers folder. The stack folder is not needed and overcomplicates things. The same goes for `infrastructure > services > stack`.
- [x] ~~Under `infrastructure > flux`, the individual YAMLs~~ **DONE (#579).** Folded into `infrastructure/configs`. need to be folded into the individual apps, or placed within infrastructure `configs`, `controllers`, or `services` where applicable.
...
- [ ] migrate `minio` to `seaweedfs`. **BLOCKED, and the blocker is capacity, not effort.** No SeaweedFS
  manifests exist anywhere — this is a greenfield deployment plus a data move, not a migration. MinIO is a
  single 256Mi pod backed by a hostPath that is itself an NFS mount on the Proxmox host; SeaweedFS is four
  components (master + volume + filer + S3 gateway) and a cutover needs both alive at once, on a node with
  no memory to spare. The live data is 62GiB `thanos-metrics` (up to a year of downsampled history that
  exists nowhere else — Prometheus keeps 7d), 1.7GiB `loki-chunks` (self-replacing within its 168h
  retention, effectively disposable) and a dead 14GiB `postgres-backups` whose newest object is 2026-05-31,
  since all three CNPG clusters now archive to OCI directly. Do the offsite mirror first: it currently
  covers 2% of thanos-metrics and last ran 2026-07-08, so there is no rollback today.
- [ ] any workload that has a persistent volume (except databases) must be moved to longhorn. **NEEDS
  REWORDING — impossible as written.** 20.9 TiB of the PVCs are bulk media (`media/movies` 10.24 TiB,
  `media/series` 6.42 TiB, `backup/timemachine-share` 2.87 TiB, plus downloads/library), which exceeds
  Longhorn's entire schedulable budget many times over and would serve no purpose: they are re-downloadable
  and already sit on the NAS. Two of them are RWX with 7-8 concurrent mounters, which Longhorn serves
  through a share-manager NFS pod — a different failure domain and performance profile from the current
  direct NFS. Restate as "non-media PVCs", or drop. Data locality was separately fixed (#593) and the nine
  cross-site volumes are the real storage problem, not the storage class.
- [x] ~~figure out an easier way to apply resource requests and limits to workloads in code.~~ **MOSTLY DONE.** `components/resource-profiles` deleted, values inlined from measurement, and a `LimitRange` now exists in all 40 namespaces. The VPA-recommender-to-PR automation is the one part not built. The industry-standard shape is four separate concerns — don't solve them with one mechanism:
  - [x] **Shape.** Set **CPU requests, omit CPU limits** — CPU is compressible, so the request already guarantees a share under contention, while a limit causes CFS throttling even on an idle node. Set **memory request == memory limit** — memory is incompressible, so a missing limit lets one leak OOM the node, and equal values mean the pod never exceeds what it reserved, which ranks it last for node-pressure eviction. Note this yields **Burstable** QoS, *not* Guaranteed: Guaranteed additionally requires cpu request == cpu limit, which the previous sentence deliberately gives up. Do not "fix" that by adding CPU limits back — the only case that genuinely needs Guaranteed is the static CPU-manager policy for exclusive cores, which nothing here uses. Today's `components/resource-profiles/*` do the opposite on both counts: they set a CPU limit and a memory limit ≠ request.
  - [x] **Values from measurement, not guesses.** Keep using KRR, or add VPA in recommendation-only (`updateMode: "Off"`) mode. Goldilocks is just a UI over VPA — not needed if KRR output is already readable.
  - [x] **Keep GitOps authoritative.** Do *not* run VPA in `Auto`/`Recreate` — it mutates live pod specs and Flux will fight it (same failure mode as any live patch to a Flux-managed value). The GitOps-native loop is: recommender → PR → review → merge → Flux applies. Worth automating the PR step, since that's the part that actually made previous right-sizing passes expensive.
  - [x] **Floor, not ceiling.** A `LimitRange` per namespace supplies a default so nothing lands unbounded, and a Kyverno `validate` rule requires requests to be present. This is what t-shirt sizes are actually for — a sane default for a *new* workload — not the steady-state value for a measured one. There are currently no `LimitRange` or `ResourceQuota` objects in the repo at all.
  - [x] **Fix or drop `components/resource-profiles`.** ~40 profiles (t/c/m/r/p × nano→2xlarge), adopted by 10 of ~139 live workloads. Every profile patches `containers/0` only, so on any multi-container pod it silently sizes the first container and ignores the rest — which is most of what the last KRR run flagged (litellm + auth-proxy, defectdojo-django + nginx, github-runner's two containers, the prometheus sidecars). Given the shape guidance above, prefer deleting it in favour of `LimitRange` defaults + measured per-workload values in git.
...
The below monstrosity should rather be put into magmamoose/tremvok, where it'll become a marketplace action for infrastructure deployments, much like how Diateme was created for Release Management. Then this infra repo must consume/call the Marketplace/Composite GitHub Action.
